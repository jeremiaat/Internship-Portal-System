...
class GradeAPITests(BaseTestCase):
    ...
    def test_reject_grade(self):
        ...
        reg_user, _ = self.create_registrar()
        self.authenticate_user(reg_user)
        url = reverse('reject_grade', kwargs={'grade_id': grade.id})
        data = {'comments': 'Insufficient work'}
        response = self.client.put(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        grade.refresh_from_db()
        self.assertEqual(grade.status, 'rejected')

    def test_only_pending_grade_can_be_approved(self):
        _, student = self.create_student()
        _, company = self.create_company()
        internship = self.create_internship(company)
        application = self.create_application(student, internship)
        grade = self.create_grade(student, application, status='approved')
        reg_user, _ = self.create_registrar()
        self.authenticate_user(reg_user)
        url = reverse('approve_grade', kwargs={'grade_id': grade.id})
        response = self.client.put(url, {}, format='json')
    ...
    def test_student_cannot_approve_grade(self):
        user, student = self.create_student()
        _, company = self.create_company()
        internship = self.create_internship(company)
        application = self.create_application(student, internship)
...
class GradeAppealAPITests(BaseTestCase):
    """Unit tests for Grade Appeal API endpoints."""

    def test_student_can_create_appeal(self):
        user, student = self.create_student()
        _, company = self.create_company()
        internship = self.create_internship(company)
        application = self.create_application(student, internship)
